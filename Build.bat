...
echo quote rcmd CHGPFM FILE(%PUB400_LIB%/QCBLLESRC) MBR(%PUB400_PGM%) SRCTYPE(CBLLE) TEXT('%BUILD_TAG% %PUB400_TEXT%') >> ftpcmd.dat

echo quote rcmd SBMJOB CMD(CRTBNDCBL PGM(%PUB400_LIB%/%PUB400_PGM%) SRCFILE(%PUB400_LIB%/QCBLLESRC) SRCMBR(%PUB400_PGM%) REPLACE(*YES) OPTION(*EVENTF) DBGVIEW(*SOURCE)) JOBD(*USRPRF) >> ftpcmd.dat
